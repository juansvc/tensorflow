...
def predict_unseen_data():
	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			...
			# Save the clasification back to file
			df['NEW_PREDICTED'] = predict_labels
			columns = sorted(df.columns, reverse=True)
			df.to_csv(predicted_dir + 'clasification.csv', index=False, columns=columns, sep='|')

			if y_test is not None:
				y_test = np.array(np.argmax(y_test, axis=1))
				accuracy = sum(np.array(predictions) == y_test) / float(len(y_test))
				logging.critical('The clasification accuracy is: {}'.format(accuracy))

			logging.critical('Clasification is complete, all files have been saved: {}'.format(predicted_dir))

if __name__ == '__main__':
	# python3 eval.py ./trained_results_1478563595/ ./data/dataset.csv
	predict_unseen_data()
