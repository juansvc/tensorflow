...
def train_cnn_rnn():
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			dev_summary_writer = tf.summary.FileWriter("tmp/dev", sess.graph)
			...
			def dev_step(x_batch, y_batch, writer=None):
				...
				step, summaries, loss, accuracy, num_correct, predictions = sess.run(
					[global_step, dev_summary_op, cnn_rnn.loss, cnn_rnn.accuracy, cnn_rnn.num_correct, cnn_rnn.predictions], feed_dict)
				if writer:
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					...
					dev_step(x_train_batch, y_train_batch, writer=dev_summary_writer)
					total_dev_correct = 0
